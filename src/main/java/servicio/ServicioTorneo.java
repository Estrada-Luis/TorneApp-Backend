...
public class ServicioTorneo {
    ...
    /**
     * ✅ CREAR UN NUEVO TORNEO DESDE EL MÓVIL
     * Ruta: api/torneos/crear/{idClub}
     */
    @PostMapping("/torneos/crear/{idClub}")
    public ResponseEntity<Torneo> crearTorneoDesdeMovil(@RequestBody Torneo torneo, @PathVariable int idClub) {
        try {
            // Importante: Usamos un ClubDAO para buscar el club y asignarlo
            dao.ClubDAO clubDAO = new dao.ClubDAO();
            Club club = clubDAO.read(idClub);
            
            if (club == null) {
                return ResponseEntity.badRequest().build();
            }
            
            // Asignamos el club y el estado inicial
