...
public class ServicioClub {
    ...
    public ResponseEntity<String> actualizar(@RequestBody Club clubMovil) {
        try {
            // 1. Buscamos el club real que ya existe en la base de datos por su ID
            Club clubDB = clubDAO.read(clubMovil.getIdClub());
            
            if (clubDB == null) {
                return ResponseEntity.status(404).body("Error: Club no encontrado con ID: " + clubMovil.getIdClub());
            }

            // 2. Solo actualizamos los campos que el usuario puede cambiar en el perfil
            // No tocamos el CIF (que ya pusiste como updatable=false) ni el Correo si no quieres
            clubDB.setNombre(clubMovil.getNombre());
            clubDB.setDireccion(clubMovil.getDireccion());
            clubDB.setTelefono(clubMovil.getTelefono());
            
            // 3. Mantenemos el estado de validación que mande el móvil
            clubDB.setValidado(clubMovil.isValidado());
            clubDB.setMotivoRechazo(clubMovil.getMotivoRechazo());

            // 4. IMPORTANTE: Si la federación viene del móvil, la actualizamos
            if (clubMovil.getFederacion() != null && clubMovil.getFederacion().getIdFederacion() != 0) {
                clubDB.setFederacion(clubMovil.getFederacion());
            }

            // NOTA: No tocamos clubDB.setPassword(...) ni clubDB.setCif(...) 
            // Así nos aseguramos de que no se borren ni den error de duplicado.

            clubDAO.update(clubDB);
            
            System.out.println("[BACKEND] Perfil actualizado para: " + clubDB.getCorreo());
