...
public class ServicioClub {
    ...
    public ResponseEntity<String> actualizar(@RequestBody Club clubMovil) {
        try {
            ...
            if (clubDB == null) return ResponseEntity.status(404).body("No encontrado");
            ...
            // 2. 🛡️ LIMPIEZA EN BD (Añade estas dos líneas)
            clubDB.setValidado(false);      // Vuelve a estar pendiente
            clubDB.setMotivoRechazo(null);  // Borramos la frase de rechazo

            clubDAO.update(clubDB);
            
            return ResponseEntity.ok("OK_RESETEADO"); 
        } catch (Exception e) {
